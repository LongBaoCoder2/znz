  ...
  sfuListener.on("connection", (socket: any) => {
    ...
    socket.on("member:getAll", (data: any, callback: any) => {
      ...
      if (allMember) {
        ...
        callback(
          Object.values(allMember).filter((member) => member.status === "approved" && member.socketId !== socket.id),
          null
        );
      } else {
    ...
    socket.on(
      "consumeAdd",
      async (data: { kind: "video" | "audio"; rtpCapabilities: any; producerId: any }, callback: any) => {
        const roomId = getRoomId();
        const clientId = socket.id;
        const { kind, rtpCapabilities, producerId } = data;

        const transport = mediasoupService.getConsumerTransport(roomId, clientId);
        if (!transport) {
        ...
        const { consumer, params }: any = await mediasoupService.createConsumer(
          roomId,
          transport,
          producer,
          rtpCapabilities
    ...
    socket.on("resumeAdd", async (data: { kind: "video" | "audio"; producerId: any }, callback: any) => {
      ...
      const consumer = mediasoupService.getConsumer(roomId, clientId, producerId, kind);
      if (!consumer) {
