...
export class Subscribe {
  ...
  async consumeOtherProducers(socketId: string) {
    const producers = await this.connector
      .sendRequest("getAllRemoteProducerIds", { localId: socketId })
      .catch((error: any) => {
  ...
  async consumeAdd(consumerTransport: mediaSoupTypes.Transport, socketId: any, userName: any, tkind: string) {
    ...
    const data = await this.connector
      .sendRequest("consumeAdd", { rtpCapabilities: rtpCapabilities, producerId: socketId, kind: tkind })
      .catch((err: any) => {
    ...
    if (kind === "video") {
      ...
      this.connector
        .sendRequest("resumeAdd", { producerId: socketId, kind: kind })
        .then(() => {
